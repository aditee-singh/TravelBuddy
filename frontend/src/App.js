...
function App() {
  ...
  const [currentUser, setUser] = useState(myStorage.getItem("user"));
  const [pins , setPins] = useState([]);
  const [newPlace , setNewPlace] = useState(null);
  const [title , setTitle] = useState(null);
  const [desc , setDesc] = useState(null);
  const [rating , setRating] = useState(0);
  const [currentPlaceId , setCurrenPlaceId] = useState(null);
  ...
  return (
    <div className="App">
      ...
      mapStyle="mapbox://style/safak/cknndpyfq268f17p53nmpwira"
      ...
      transitionDuration="200">
          ...
          {p._id === currentPlaceId && (
            <Popup
              ...
              <div className="card">
                ...
                <span className="username">Created by <b className="name">{p.username}</b></span>
        ...
        {showLogin&& <Login setShowLogin={setShowLogin} myStorage={myStorage} setUser={setUser}/>}
    </ReactMapGL>
    </div>
  );
}
